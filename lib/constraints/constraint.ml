...
module Closure = struct
  ...
  let of_list vars =
    let type_vars, vars =
      List.partition_map vars ~f:(function
        | `T type_var -> Either.First type_var
        | `C var -> Second var)
    in
    { type_vars = Type.Var.Set.of_list type_vars; vars = Var.Set.of_list vars }
  ;;
end
