...
let parse_and_print lexbuf =
  parse lexbuf @@ fun structure -> Fmt.pr "%a@." pp_structure structure
;;

let constraint_gen lexbuf ~dump_ast k =
  parse lexbuf
  @@ fun structure ->
...
let constraint_gen_and_print lexbuf ~dump_ast =
  constraint_gen lexbuf ~dump_ast @@ fun cst -> Fmt.pr "%a@." pp_constraint cst
;;

let type_check_and_print lexbuf ~dump_ast ~dump_constraint =
  constraint_gen lexbuf ~dump_ast
  @@ fun cst ->
